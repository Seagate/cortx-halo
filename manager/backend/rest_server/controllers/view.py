...
class MgmtAccess:
    ...
    def roles(cls, roles):
        ...
        def decorator(handler):
            setattr(handler, cls.ATTR_ROLES, roles)
            return handler

        return decorator

    @classmethod
    def get_roles(cls, handler):
        return getattr(handler, cls.ATTR_ROLES, [])
...
class MgmtWebResponse(web.Response):
...
class MgmtHttpException(web.HTTPException):
...
class MgmtView(web.View):

    # derived class will provide service amd service_dispatch  details
    _service = None
    _service_dispatch = {}
    ...
    def __init__(self, request):
        super(MgmtView, self).__init__(request)
    ...
    def is_public(cls, handler, method):
        """
        Check whether a particular method of the MgmtView subclass has the 'public' attribute.
        ...
        if method_handler is not None:
            if MgmtAccess.is_public(method_handler):
                return True
        return MgmtAccess.is_public(handler)

    @classmethod
    def is_hybrid(cls, handler, method):
        method_handler = cls._get_method_handler(handler, method)
        if method_handler is not None:
            if MgmtAccess.is_hybrid(method_handler):
                return True
        return MgmtAccess.is_hybrid(handler)
    ...
    def get_permissions(cls, handler, method):
        ...
        Combine required permissions from the individual method handler (like get/post/...)
        and from the whole view.
        """
        view_permissions = MgmtAccess.get_permissions(handler)
        method_handler = cls._get_method_handler(handler, method)
        if method_handler is not None:
            method_permissions = MgmtAccess.get_permissions(method_handler)
    ...
    @classmethod
    def get_roles(cls, handler, method):
        """
        Obtain the list of required permissions associated with the handler.

        Combine required user roles from the individual method handler (like get/post/...)
        and from the whole view.
        """
        view_roles = MgmtAccess.get_roles(handler)
        method_handler = cls._get_method_handler(handler, method)
        if method_handler is not None:
            method_roles = MgmtAccess.get_roles(method_handler)
            # TODO: Merge view and method permissions?
            # permissions = view_permissions | method_permissions
            roles = method_roles
        else:
    ...
    async def parse_multipart_request(self,
        ...
        if not os.path.exists(file_cache.cache_dir):
            try:
                ...
                raise MgmtInternalError(f"System error during directory creation for "
